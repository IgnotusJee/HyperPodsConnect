...
class HeadphoneIpcCodecTest {
    ...
    @Test
    fun `IPC normalizes every boolean capability without changing enum values`() {
        val identity = DeviceIdentity(
            DeviceId("device:boolean-contract"),
            VendorId.OPPO,
            "11:22:33:44:55:66",
        )
        fun capability(
        ...
        val booleanIds = listOf(
            FeatureId.TRANSPARENCY_VOCAL_ENHANCEMENT,
            FeatureId.LOW_LATENCY,
            FeatureId.SPATIAL_SOUND_SWITCH,
            FeatureId.DUAL_DEVICE_CONNECTION,
        )
        val features = booleanIds.associateWith(::capability) +
            (FeatureId.NOISE_CONTROL to capability(
                FeatureId.NOISE_CONTROL,
                linkedSetOf("OFF", "TRANSPARENCY"),
            ))
        val profile = DeviceProfile(
            identity = identity,
            ...
            protocol = ProtocolDescriptor("test"),
            features = features,
            compatibilityLevel = CompatibilityLevel.CONTROLLED,
        )
        ...
        val payload = HeadphoneSnapshotPayload.from(
            HeadphoneSnapshot(
                ...
                generationId = 1,
                connection = SessionState.Ready(identity.id, 1, TransportKind.CLASSIC_SPP),
                profile = profile,
                emittedAtMillis = 1,
            ),
            "host-boolean",
        )
        val valuesByFeature = payload.capabilities.associate {
            it.featureId to it.allowedValues
        }

        booleanIds.forEach { featureId ->
            assertEquals(listOf("false", "true"), valuesByFeature[featureId.name])
        }
        assertEquals(
            listOf("OFF", "TRANSPARENCY"),
            valuesByFeature[FeatureId.NOISE_CONTROL.name],
        )
    }
