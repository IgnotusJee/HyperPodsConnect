...
abstract class HookContext {
    ...
    fun hyperOsPresentationTypeId(): String = HyperOsOfficialIslandConfig.presentationTypeId

    fun hyperOsSupportDescriptor(): String = HyperOsOfficialIslandConfig.supportDescriptor
    ...
    fun hookBefore(method: Method, block: HookParam.() -> Unit) {
        module.hook(method).intercept { chain ->
            ...
            when {
                ...
                param.changedArgs != null -> chain.proceed(param.changedArgs!!.toTypedArray())
                else -> chain.proceed()
            }
        }
    }
...
class HookParam(private val chain: XposedInterface.Chain, initialResult: Any?) {
    ...
    internal var changedArgs: MutableList<Any?>? = null
        private set

    fun setArg(index: Int, value: Any?) {
        val mutable = changedArgs ?: chain.args.toMutableList().also { changedArgs = it }
        mutable[index] = value
    }
}
