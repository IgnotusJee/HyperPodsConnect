...
fun HeadphoneUiState.toOfficialHeadsetIslandPayload(): OfficialHeadsetIslandPayload? {
    return toPresentationState().toOfficialHeadsetIslandPayload()
}

fun HeadphonePresentationState.toOfficialHeadsetIslandPayload(): OfficialHeadsetIslandPayload? {
    if (!connected || deviceId == null || address.isNullOrBlank()) return null

    val single = batteries[PresentationBatterySlot.SINGLE]
    val left = batteries[PresentationBatterySlot.LEFT] ?: single
    val right = batteries[PresentationBatterySlot.RIGHT]
...
fun HeadphoneUiState.officialIslandEmissionToken(): String? {
    val payload = toOfficialHeadsetIslandPayload() ?: return null
    val normalizedAddress = address?.trim()?.uppercase() ?: return null
    return "$generationId:$normalizedAddress:${payload.wearState}"
}
...
fun shouldTriggerOfficialHeadsetIsland(
    ...
    val previousPayload = previous.toOfficialHeadsetIslandPayload() ?: return false
    val currentPayload = current.toOfficialHeadsetIslandPayload() ?: return false
    return previousPayload.wearState != currentPayload.wearState
}
...
private fun HeadphonePresentationState.officialWearState(): Int {
    if (wearing.isEmpty()) return OfficialHeadsetIslandPayload.WEAR_NOT_SUPPORTED

    val single = wearing[PresentationBatterySlot.SINGLE]
    if (single != null) {
    ...
    val left = wearing[PresentationBatterySlot.LEFT]?.isWearing() == true
    val right = wearing[PresentationBatterySlot.RIGHT]?.isWearing() == true
    return when {
