...
fun connectionPresentationKey(
    ...
    val normalizedAddress = address?.trim()?.takeIf(String::isNotEmpty)?.uppercase()
    if (normalizedAddress != null) return "address:$normalizedAddress"
    return deviceName?.trim()?.takeIf(String::isNotEmpty)?.lowercase()
        ?.let { "model:$vendor:$it" }
}
