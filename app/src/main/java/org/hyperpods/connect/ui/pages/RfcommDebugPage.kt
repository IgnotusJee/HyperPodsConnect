...
fun RfcommDebugPage(
    ...
    DisposableEffect(context) {
        ...
        context.registerReceiver(receiver, IntentFilter(HeadphoneActionContract.ACTION_RFCOMM_LOG), Context.RECEIVER_EXPORTED)
        context.sendRfcommDebugBroadcast(HeadphoneActionContract.ACTION_RFCOMM_LOG_CONNECT)
        onDispose {
            context.sendRfcommDebugBroadcast(HeadphoneActionContract.ACTION_RFCOMM_DEBUG_LOCK) {
                putExtra(HeadphoneActionContract.EXTRA_RFCOMM_DEBUG_SESSION_TOKEN, rawHexSessionToken)
            }
            context.sendRfcommDebugBroadcast(HeadphoneActionContract.ACTION_RFCOMM_LOG_DISCONNECT)
    ...
    LaunchedEffect(clearRequest) {
        if (clearRequest > 0) {
            logs.clear()
            context.sendRfcommDebugBroadcast(HeadphoneActionContract.ACTION_RFCOMM_LOG_CLEAR)
        }
    }
    ...
    Column(
        ...
        when {
            ...
            !rawHexUnlocked -> RawHexUnlockCard {
                context.sendRfcommDebugBroadcast(HeadphoneActionContract.ACTION_RFCOMM_DEBUG_UNLOCK) {
                    putExtra(HeadphoneActionContract.EXTRA_RFCOMM_DEBUG_SESSION_TOKEN, rawHexSessionToken)
                }
                rawHexUnlocked = true
            }
                ...
                TextButton(
                    text = "发送",
                    onClick = {
                        context.sendRfcommDebugBroadcast(HeadphoneActionContract.ACTION_RFCOMM_DEBUG_SEND) {
